 ...
 */
export const getCircle = (dia) => {
    if (circles.has(dia))
        return circles.get(dia);
    const result = [];
    for (let x = 0; x < dia; x++) {
        for (let y = 0; y < dia; y++) {
 ...
 */
export function getCircleFrom([x0, y0], dia, mapWidth) {
 ...
 */
export function getCircleIndexes(center, dia, dimensions) {
    ...
    return getI(center[0], x)
        .flatMap((x) => getI(center[1], y).map((y) => [x, y]))
        .map(([x, y]) => index([y, x]));
}
function index(x, y, mapWidth) {
    return y * mapWidth + x;
}
async function bufferToBase64(buffer) {
    return await new Promise((r) => {
        const reader = new FileReader();
        reader.onload = () => r(reader.result.substring(37));
 ...
 */
export async function compressBitMap(arr) {
 ...
 */
export async function decompressBitMap(base64) {
...
export function createBitMapForMap(dimensions) {
 ...
 */
export function calcIndexInPaintMap(domain, item, mapWidth) {
    if (item >= domain[1])
        return mapWidth - 1;
    if (item <= domain[0])
        return 0;
 ...
 */
export function calcIndexesInPaintMap(dataX, dataY, domainX, domainY, mapWidth) {
 ...
 * @returns
 */
export async function calcPaintMap(dataX, dataY, paintMap) {
...
// e.g. [3,3,3] => [9,3,1]
function indexByDimensions(dimensions) {
    const indexWeights = dimensions
        .map((x) => x.domain.width)
        .reduceRight(([n, ...rest], a) => [a * n, n, ...rest], [1])
        .slice(1);
    return (indexes) => indexes.map((i, wi) => i * indexWeights[wi]).reduce((x, y) => x + y);
}
/**
 ...
 */
export function calcIndexesByDimensions(dimensions) {
    const getSingleIndex = dimensions.map(({ domain, fid }) => {
        if (domain.type === 'nominal') {
            const indexDict = new Map(domain.value.map((x, i) => [`${x}`, i]));
            return (data) => indexDict.get(`${data[fid]}`) ?? 0;
        }
        if (domain.type === 'quantitative') {
            return (data) => calcIndexInPaintMap(domain.value, data[fid], domain.width);
        }
        const neverType = domain;
        throw new Error(`unsupported domain type ${neverType['type']}`);
    });
    const index = indexByDimensions(dimensions);
    return (data) => index(getSingleIndex.map((f) => f(data)));
}
export function IPaintMapAdapter(paintMap) {
    return {
        dict: paintMap.dict,
 ...
 * @returns result
 */
export async function calcPaintMapV2(data, paintMap) {
