...
export function parseSQLExpr(sql) {
...
export function getSQLItemAnalyticType(item, fields) {
    const map = new Map();
    fields.forEach((f) => {
...
export function replaceFid(sql, fields) {
    const dict = new Map();
    fields.forEach((f) => {
...
const isSingleTruly = (x) => {
    if (x instanceof Array) {
        return !!x[0];
    }
    return !!x;
};
function exprSQL(item, datas) {
    const exec = (i) => {
        switch (i.type) {
            case 'ref': {
            ...
            case 'case': {
                if (datas instanceof Array) {
                    if (datas.length === 1) {
                        const sat = i.whens.find((x) => isSingleTruly(exec(x.when)));
                        if (sat) {
...
export function expr(sql, datas) {
