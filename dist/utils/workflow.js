...
const walkExpression = (expression, each) => {
...
const deduper = (items, keyF) => {
    const map = new Map();
    items.forEach((x) => map.set(keyF(x), x));
    return [...map.values()];
};
const treeShake = (computedFields, viewKeys) => {
...
export const toWorkflow = (viewFilters, allFields, viewDimensionsRaw, viewMeasuresRaw, defaultAggregated, sort, folds = [], limit, timezoneDisplayOffset) => {
    ...
    const viewKeys = new Set([...viewDimensions, ...viewMeasures, ...viewFilters].map((f) => f.fid));
    let filterWorkflow = null;
    let transformWorkflow = null;
    let computedWorkflow = null;
    let viewQueryWorkflow = null;
    let sortWorkflow = null;
    let aggFilterWorkflow = null;
    ...
    const filters = viewFilters.filter((f) => !f.computed && f.rule && !f.enableAgg).map(buildFilter);
    if (filters.length) {
        filterWorkflow = {
            type: 'filter',
            filters,
    ...
    const computedFields = treeShake(allFields
    ...
    const computedFilters = viewFilters.filter((f) => f.computed && f.rule && !f.enableAgg).map(buildFilter);
    ...
    const aggergatedComputed = treeShake(allFields
        .filter((f) => f.computed && f.expression && f.expression.op === 'expr' && f.aggName === 'expr')
        .map((f) => {
        return {
            key: f.fid,
    ...
    const aggregateOn = viewMeasures
        .filter((f) => f.aggName)
        .map((f) => [f.fid, f.aggName])
        .concat(aggergatedFilter.map((f) => [f.fid, f.aggName]))
            ...
            query: [
                {
                    ...
                    measures: deduper(viewMeasures
                        ...
                        .concat(aggergatedComputed.map((f) => ({
...
export const addTransformForQuery = (query, transform) => {
    ...
    const transformQuery = { type: 'transform', transform };
    return { ...query, workflow: [transformQuery, ...query.workflow] };
};
export const addFilterForQuery = (query, filters) => {
    if (filters.length === 0)
        return query;
    ...
    const filterQuery = { type: 'filter', filters };
    return {
        ...query,
...
export function specToWorkflow(chart) {
    const parsedChart = parseChart(chart);
    const viewEncodingFields = viewEncodingKeys(parsedChart.config?.geoms?.[0] ?? 'auto').flatMap((k) => parsedChart.encodings?.[k] ?? []);
    const rows = parsedChart.encodings?.rows ?? [];
    const columns = parsedChart.encodings?.columns ?? [];
    const limit = parsedChart.config?.limit ?? -1;
    return {
        workflow: toWorkflow(parsedChart.encodings?.filters ?? [], [...(parsedChart.encodings?.dimensions ?? []), ...(parsedChart.encodings?.measures ?? [])], viewEncodingFields.filter((x) => x.analyticType === 'dimension'), viewEncodingFields.filter((x) => x.analyticType === 'measure'), parsedChart.config?.defaultAggregated ?? true, getSort({ rows, columns }), parsedChart.config?.folds ?? [], limit, parsedChart.config?.timezoneDisplayOffset),
        limit: limit > 0 ? limit : undefined,
    };
}
export const processExpression = (exp, allFields, config) => {
    if (exp?.op === 'expr') {
        return {
            ...exp,
            params: [
                ...
                if (x.type === 'map') {
                    ...
                    return {
                        ...
                        value: {
                            ...
                            map: x.value.map,
                            dict: Object.fromEntries(x.value.usedColor.map((i) => [
                                i,
                                {
                                    name: dict[i].name,
                                },
                    ...
                    return {
                        ...
                        value: {
                            ...
                            dict: Object.fromEntries(colors.map((i) => [
                                i,
                                {
                                    name: dict[i].name,
                                },
