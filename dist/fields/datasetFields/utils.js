...
const keepTrue = (array) => {
    return array.filter(Boolean);
};
export const useMenuActions = (channel) => {
        ...
        return fields.map((f, index) => {
            ...
            return keepTrue([
                ...
                },
                {
                    ...
                    disabled: isInnerField,
                    children: ['nominal', 'ordinal', 'quantitative', 'temporal'].map((x) => ({
                ...
                (f.semanticType === 'temporal' || isDateTimeDrilled) && {
                    ...
                    children: DATE_TIME_DRILL_LEVELS.map((level) => ({
                        ...
                        onPress() {
                            const originField = (isDateTimeDrilled
                            ...
                            getSample(computation, originField.fid)
                                .then(getTimeFormat)
                                .then((format) => vizStore.createDateTimeDrilledField(originChannel, originIndex, level, `${t(`drill.levels.${level}`)} (${originField.name || originField.fid})`, format, f.offset));
                        },
                    })),
                },
                (f.semanticType === 'temporal' || isDateTimeDrilled) && {
                    ...
                    children: DATE_TIME_FEATURE_LEVELS.map((level) => ({
                        ...
                        onPress() {
                            ...
                            getSample(computation, originField.fid)
                                .then(getTimeFormat)
                                .then((format) => vizStore.createDateFeatureField(originChannel, originIndex, level, `${t(`drill.levels.${level}`)} [${originField.name || originField.fid}]`, format, f.offset));
                        },
                    })),
                },
                {
